...
foreach my $samplefile (@samplefiles) {
	...
	my $mirna_file = "miRNA.txt"; #miRNA.txt does not have counts from crossmapped.txt, but mirna_species.txt does
	my $cross_file = "crossmapped.txt";
	my $isoform_file = "isoforms.txt";
	...
	unless (-d "$featuredir/tcga") {
		make_path "$featuredir/tcga" or die "Could not make tcga subfolder $featuredir/tcga: $!";
	}
